...
try{
	...
	System.out.println("SetKemu1Test.jsp "+e);
}
	...
	<div id="left">
		...
		<div class="navi">
       		<ul>
                ...
                <li><a href="SetKemu4Test.jsp">科目四</a></li>
        	</ul>
    	</div>
...
<%
				} else if(lastScoreKemu1<90){
%>
			    	你已参加过科目一考试。<br />
			    	成绩：<%=lastScoreKemu1 %>。<font style="color:red;">不合格！</font><br />
			    	请联系老师申请重新参加考试。
			    	<div class="height"></div>
...
%>
					你已参加过科目一考试，不得重复参加。<br />
			    	成绩：<%=lastScoreKemu1 %>。合格。
			    	<div class="height"></div>
...
<script>
function BeginTest(){
	var a = confirm("你即将开始科目一考试。考试限时45分钟，一旦开始，中途不得暂停，不得重复参加考试。是否确认开始考试？");
	if(!a) {
...
function NoMoreTest(){
	alert("你已参加过科目一考试，不得重复参加。");
}
function NoMoreTestFail(){
	alert("你已参加过科目一考试，未及格，请联系老师申请重新参加考试。");
}
</script>	
