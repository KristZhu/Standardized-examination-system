...
if(lastScoreKemu1>=0){
	if(isStudent) response.sendRedirect("SetKemu1Test.jsp");
...
if(number<=1){
	...
	try{
		...
		sql.executeUpdate("UPDATE student SET scoreKemu1 = 0 WHERE id = " + id);
	} catch(Exception e){
		System.out.println("Kemu1Test.jsp "+e);
	}	
}
...
%>
                        ...
                        <div class="others">
                        	<div class="align"></div>                            
                            <div class="other"><a href="FinishKemu1Test.jsp" id="submitTest" onclick="return DoFinish();"><img src="images/student/finishTestButton.png" /></a></div>
                        </div>
                                      
                        <div class="cleaner"></div>
                        <div id="autoSubmit">
                        	<form id="autoSubmit" action="autoSubmitServlet" method=post name="autoSubmit">
                        		<input type="hidden" name="whereToGo" value="kemu1Test" />
                        		<input type="hidden" id="hiddenTimeCounterMin2" name="timeCounterMin">
		                    	<input type="hidden" id="hiddenTimeCounterSec2" name="timeCounterSec">
                    			<input type="hidden" value="<%=testBean.getId()%>" name ="id">
<%								
...
<script>
	...
	if(minute==0&&second==0) minute=45;	
	var init;  
	function startTimer(){
